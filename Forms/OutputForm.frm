...
Private Sub SubmitBtn_Click()
    ...
    startDate = DateSerial(CInt(OutputForm.txtYear1.Value), CInt(OutputForm.txtMonth1.Value), CInt(OutputForm.txtDay1.Value))
    endDate = DateSerial(CInt(OutputForm.txtYear2.Value), CInt(OutputForm.txtMonth2.Value), CInt(OutputForm.txtDay2.Value))
    On Error GoTo 0
    ...
    ' Update text boxes in the Dashboard sheet with formatted dates
    dashboardSheet.Shapes("StartDateTextBox").TextFrame2.TextRange.Text = Format(startDate, "mmmm d, yyyy")
    dashboardSheet.Shapes("EndDateTextBox").TextFrame2.TextRange.Text = Format(endDate, "mmmm d, yyyy")
    ...
    MsgBox "Data has been successfully filtered and net worth updated.", vbInformation

    Exit Sub

DateError:
    MsgBox "Please enter valid start and end dates in the OutputForm.", vbExclamation
End Sub
